...
class ShapesCollection:
    ...
    def dynamic_shapes(self, m, args, kwargs=None):
        ...
        if any(t_id not in t_ids for t_id in self._shapes):
            raise ValueError(
                "Some tensors that were assigned shapes do not appear in args. "
                "This is usually a bug, since the intended shapes could be dynamic and we would now assume them to be static. "
...
def _process_dynamic_shapes(
    ...
    if not isinstance(dynamic_shapes, dict):
        ...
        combined_args = type(dynamic_shapes)(combined_args.values())  # type: ignore[assignment]
